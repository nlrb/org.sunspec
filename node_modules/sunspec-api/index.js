...
class SunSpec extends Events {
	...
	constructor(ip, port, mbId, mbBase, debug) {
		...
		this.on('COM.SN', () => {
			this.emit('found', {
				found: true,
				id: this.values['COM.SN'],
				name: this.values['COM.Mn'] + ' ' + this.values['COM.Md'],
				show: [ // TODO: support locales
					{ val: this.values['COM.Mn'], name: 'Manufacturer' },
					{ val: this.values['COM.Md'], name: 'Model' },
					{ val: this.values['COM.SN'], name: 'Serial number' },
					{ val: this.values['COM.Ver'], name: 'Version' }
				]
			});
		});
		this.on('found', data => {
					...
					if (this.modBusScan == true) {
						...
						if (sun.map.DID[did] != null && sun.map.DID[did].model != null) {
							...
							if (model == 'inverterModel') {
								this.emit('measured', this.values['INV.W']);
							}
						}
					}
				} else {
