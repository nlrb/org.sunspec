...
class SunSpec extends Events {
	...
	constructor(ip, port, mbId, mbBase, debug) {
		...
		this.found = false;
		// Message handling
		this.modbus = new modbus(this.tcp);
		this.msgModBus = Buffer.from([]);
		this.msgLen = 0;
		...
		if (!this.client.destroyed) {
			for (let key in this.sendMsg) {
				...
				if (val.poll != null && val.poll == true) {
					this.debug("Poll request for", key, "of length,", 2 * this.sendMsg[key].len);
	...
	sendRequest(id) {
		...
		if (this.responseQueue.length == 0) {
			...
			if (msg != null) {
				let req = new Buffer(this.modbus.makeRequest(this.settings.mbId, 0x03, this.settings.mbBase + msg.addr, msg.len));
	...
	processData(data) {
		...
		if (this.msgModBus.length === expectedLen) {
			...
			if (crcOk && this.responseQueue.length > 0) { // only process data if we are expecting a response
				...
				let idx = 3 + 6 * this.tcp;
				if (this.msgLen == len * 2) {
					let did = ('000' + sun.decode.value("uint16", this.msgModBus.slice(idx, idx + 2))).slice(-3);
					if (this.modBusScan == true) {
						if (resp != null) {
							if (resp == this.msgModBus.slice(idx, idx + resp.length).toString()) {
							...
							let len = sun.decode.value("uint16", this.msgModBus.slice(idx + 2, idx + 4));
						...
						if (sun.map.DID[did] != null && sun.map.DID[did].model != null) {
							...
							let modelValues = sun.decode.model(model, this.msgModBus.slice(idx));
			...
			this.msgModBus = Buffer.from([]);
		} else if (this.msgModBus.length > expectedLen) {
			this.debug("Received message too long, discarding");
			this.msgModBus = Buffer.from([]);
		}
